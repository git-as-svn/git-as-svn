...
. Add `git-as-svn:<branch>` topics to whatever repositories you want to expose via git-as-svn via "Settings -> General -> Topics" in GitLab project settings.
For example, add `git-as-svn:master` will expose `master` branch. If you want to expose multiple branches, add multiple topics separated by commas.
