...
. Add variables required for GitLab hooks to `/etc/default/git-as-svn`:
+
[source,bash]
----
...
If you want to expose more than one branch, add multiple `git-a-svn:<branch>` topics separated by commas.

== Supported Git LFS modes

. git-as-svn uses GitLab LFS API for all LFS operations
+
[source,yaml]
----
lfsMode: !httpLfs {}
